...
#[repr(u8)] // This enum is represented exactly as a u8
pub enum Color {
    Black = 0,
    Blue = 1,
    Green = 2,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(transparent)] // Ensure same layout as inner field (u8)
struct ColorCode(u8);
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[repr(C)] // Ensure fields are laid out in order
struct ScreenChar {
    ascii_character: u8,
...
#[repr(transparent)]
struct Buffer {
    // Use Volatile to prevent compiler optimizations that could remove writes
    // This crate provides ways to perform volatile reads and writes
    // It is important because the VGA buffer is memory-mapped I/O
...
// Implement the core::fmt::Write trait for our Writer
// This allows us to use the write! / writeln! macro with our Writer
